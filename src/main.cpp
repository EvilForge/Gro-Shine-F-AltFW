...
// If MQTT is unavilable, the device may be stuck in a loop and appear dead.
...
// Secret and password data is set in secret.h, which is ignored by git.
#include "secrets.h"

#if PINGER_SUPPORTED == 1
#define GATEWAY_IP IPAddress(10, 10, 31, 1)
#endif
...
void MqttReconnect() 
{
  ...
  while (!MqttClient.connected()) 
  {
    ...
    if (MqttClient.connect("Growatt", MQTT_USER, MQTT_PASSWORD, MQTT_TOPIC, 1, 1, "{\"Status\": \"Disconnected\" }"))
    {
      #if ENABLE_DEBUG_OUTPUT == 1
...
void CreateJson(char *Buffer)
{
  ...
  sprintf(Buffer, "%s  \"FirmwareVer\": \"%s\",\r\n",   Buffer, FIRMWARE_VERSION);
  ...
  sprintf(Buffer, "%s  \"FirmwareVer\": \"%d\",\r\n",   Buffer, FIRMWARE_VERSION);
  sprintf(Buffer, "%s  \"Cnt\": %u\r\n",                 Buffer, u16PacketCnt);
      ...
      while ((u8RetryCounter) && !(readoutSucceeded))
      {
        ...
        else
        {
          ...
          else
          {
            ...
            sprintf(JsonString, "{\"State\": \"Disconnected\" }");
            #if MQTT_SUPPORTED == 1 
            MqttClient.publish(MQTT_TOPIC, JsonString, true);
            #endif
